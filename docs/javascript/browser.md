...
整个渲染流程分为多个阶段，分别是：HTML 解析、样式计算、布局、分层、绘制、光栅化、画
每个阶段都有明确的输入输出，上一个阶段的输入会成为下一个阶段的输入，整个渲染流程形成了一套阻止严密的生产流水线

## 1.解析 HTML

解析过程中遇到 CSS 解析 CSS，遇到 JS 执行 JS。为了提高解析效率，浏览器在开始解析前，会启动一个与解析的线程，率先下载 HTML 中的外部 CSS 文件和 JS 文件
